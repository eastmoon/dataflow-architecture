...
:cli-run (
    ...
    copy python\.dependencies docker\
    docker build --rm -t process-architecture:%PROJECT_NAME% ./docker
    del docker\.dependencies

    echo ^> Startup docker container instance and execute crawler
    docker run -ti --rm^
        -v %cd%\python:/repo/^
        process-architecture:%PROJECT_NAME% bash
    goto end
)
